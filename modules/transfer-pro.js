Zuna.register({
        ...
        <div class="tool-ui">
            ...
            <div id="panel-send">
                <div id="tr-setup-send">
                    <span class="label">PILIH FILE FILM (UNLIMITED)</span>
                ...
                <div id="tr-res-send" style="display:none; margin-top:20px; text-align:center;">
                    <div style="background:#fff; color:#000; padding:25px; border-radius:30px;">
                        <span id="tr-send-status" style="font-size:10px; font-weight:800; color:var(--green); letter-spacing:1px">MENUNGGU PENERIMA...</span>
                        ...
                        <p style="font-size:10px; color:#999">JANGAN TUTUP HALAMAN INI</p>
                    </div>
                </div>

                <div id="tr-send-prog" style="display:none; margin-top:20px">
                    <span class="label" id="tr-send-info">MENGIRIM...</span>
            ...
            <div id="panel-rec" style="display:none">
                <span class="label">MASUKKAN PIN DARI PENGIRIM</span>
                ...
                <div id="tr-rec-prog" style="display:none; margin-top:30px; text-align:center">
                    <span class="label" id="tr-rec-info" style="color:var(--green)">MENERIMA DATA...</span>
                    ...
                    <p style="font-size:9px; color:var(--sub)">FILE AKAN OTOMATIS TERSIMPAN SAAT 100%</p>
                </div>
            </div>
        </div>
    `,
    logic: () => {
        let peer = null;
        let conn = null;
        const CHUNK_SIZE = 16384; // 16KB per chunk agar stabil
        ...
        document.getElementById('tr-btn-ready').onclick = function() {
            ...
            if (!file) return alert("Pilih file film dulu!");

            const pin = Math.floor(1000 + Math.random() * 9000).toString();
            const peerId = "zuna-" + pin;
            ...
            peer.on('connection', (connection) => {
                ...
                const sendNextChunk = () => {
                    if (offset < file.size) {
                        ...
                        // Gunakan timeout kecil agar browser tidak hang
            ...
            peer.on('error', (err) => {
                if (err.type === 'unavailable-id') {
                    alert("PIN sudah dipakai, silakan klik lagi untuk buat PIN baru.");
                } else {
        ...
        document.getElementById('tr-btn-connect').onclick = function() {
            ...
            if (pin.length !== 4) return alert("Input PIN 4 Angka!");

            if (peer) peer.destroy();
            peer = new Peer(); // ID Penerima bebas

            peer.on('open', () => {
                const targetId = "zuna-" + pin;
                ...
                connection.on('open', () => {
                    ...
                    document.getElementById('tr-btn-connect').innerText = "TERHUBUNG";
                });

                connection.on('data', (payload) => {
                    fileChunks.push(payload.data);
                    ...
                    if (receivedSize >= payload.size) {
                        ...
                        a.click();
                        alert("TRANSFER SELESAI! File otomatis terdownload.");
                        peer.destroy();
                    }
                });
            });
            ...
            peer.on('error', () => {
                alert("Gagal terhubung. Pastikan PIN benar dan pengirim masih standby.");
                document.getElementById('tr-btn-connect').disabled = false;
            });
        };
    }
});